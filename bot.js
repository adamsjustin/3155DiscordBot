...
async function translate(args, msg) {
  ...
  myString = args.join(" ");
  try {
    var myText = await trans(myString, {
      from: fromCase,
      to: toCase
    });
...
function mathFunc(message, args) {
  args.shift();
  if (!args[0]) return message.channel.send('Math Only');
  let resp;
  try {
    resp = math.eval(args.join(' '));
  } catch (e) {
    return message.channel.send('Please input real math.');
  }
